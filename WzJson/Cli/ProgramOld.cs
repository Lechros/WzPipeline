...
using WzJson.Domains.SetItem;
using WzJson.Domains.Soul.Converters;
using WzJson.Domains.Soul.Models;
using WzJson.Domains.Soul.Processors;
using WzJson.Domains.Soul.Traversers;
using WzJson.Domains.String;
using WzJson.Shared;
using WzJson.Shared.Exporter;
using WzJson.Shared.Json;
using WzJson.Shared.Processor;
using WzJson.Shared.Traverser;
using ItemOptionConverter = WzJson.Domains.ItemOption.ItemOptionConverter;
using SkillOptionConverter = WzJson.Domains.Soul.Converters.SkillOptionConverter;
...
public static class ProgramOld
{
    public static void MainOld(string[] args)
    {
        ...
        var nameDescDictCollector = DictionaryCollector.Create((NameDesc nd) => nd.Id);
        ...
        AnsiConsole.Live(new Text("Loading...")).Start(ctx =>
        {
            ...
            var gearStringData = Builders.LinearPipelineBuilder("Parse gear string data")
                ...
                .Processor("Collect to Dictionary",
                    DictionaryCollector.Create((NameDesc nd) => nd.Id, () => new GearNameDescData()))
                .Build()
                .Run(progress).Value;
            ...
            var skillNameData = Builders.LinearPipelineBuilder("Parse skill name data")
                ...
                .Converter("Convert to NameDesc", nameDescConverter)
                .Processor("Collect to Dictionary",
                    DictionaryCollector.Create((NameDesc nd) => nd.Id, nd => nd.Name!, () => new SkillNameData()))
                .Build()
                .Run(progress).Value;
            ...
            var itemOptionData = Builders.LinearPipelineBuilder("Parse item option data")
                ...
                .Processor("Collect to Dictionary",
                    DictionaryCollector.Create((ItemOptionEntry io) => io.Code, () => new ItemOptionData()))
                .Build()
                .Run(progress).Value;
            ...
            var consumeNameData = Builders.LinearPipelineBuilder("Parse consume name data")
                ...
                .Converter("Convert to NameDesc", nameDescConverter)
                .Processor("Collect to Dictionary",
                    DictionaryCollector.Create((NameDesc nd) => nd.Id, nd => nd.Name!, () => new ConsumeNameData()))
                .Build()
                .Run(progress).Value;
            ...
            var soulInfoData = Builders.LinearPipelineBuilder("Parse soul info data")
                ...
                .Processor("Collect to SortedDictionary",
                    DictionaryCollector.Create((SoulInfo si) => si.SoulId, () => new SoulInfoData()))
                .Build()
                .Run(progress).Value;
            ...
            var setItemCollector = DictionaryCollector.Create((MalibSetItem s) => s.Id,
            ...
            var result = Builders.GraphPipelineBuilder("Gear export pipeline")
                .Traverser("Gears", gearTraverser, t => t
                    .Converter("Convert to RawGear", gearConverter, c => c
                        .Processor("Convert to MalibGear", rawGearToMalibGearChain, p => p
                            .Processor("Collect to SortedDictionary",
                                DictionaryCollector.Create((MalibGear g) => g.Meta.Id.ToString(),
                                    () => new SortedDictionary<string, MalibGear>()), dc => dc
                ...
                .Traverser("Souls", soulTraverser, t => t
                    .Converter("Convert to MalibSoul", soulConverter, c => c
                        .Processor("Collect to SortedDictionary",
                            DictionaryCollector.Create((MalibSoul s) => s.Id,
                                () => new SortedDictionary<int, MalibSoul>()), e => e
