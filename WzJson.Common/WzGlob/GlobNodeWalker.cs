...
public class GlobNodeWalker(IWzProvider wzProvider, string pattern)
{
    private readonly List<IWzNodeSearcher> searchers = GetSearchers(pattern);
    ...
    private IEnumerable<Wz_Node> Search(Wz_Node node, int index, WzImageHolder holder)
    {
        ...
        if (index == searchers.Count)
        {
            yield return node;
            yield break;
        }

        foreach (var child in searchers[index].Search(node))
        {
    ...
    private int Count(Wz_Node node, int index, WzImageHolder holder)
    {
        ...
        if (index == searchers.Count - 1)
        {
            return searchers[index].Count(node);
        }

        return searchers[index].Search(node).Sum(child => Count(child, index + 1, holder));
    }
