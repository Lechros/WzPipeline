...
public class WzComparerR2Gear
{
    public WzComparerR2Gear()
    {
        ...
        Options = new WzComparerR2Potential[3];
        AdditionalOptions = new WzComparerR2Potential[3];
        // Additions = new List<Addition>();
    }

    public int ItemID { get; set; }
    public WzComparerR2GearGrade Grade { get; set; }
    public WzComparerR2GearGrade AdditionGrade { get; set; }
    ...
    public WzComparerR2Potential[] Options { get; private set; }
    public WzComparerR2Potential[] AdditionalOptions { get; private set; }
    ...
    {
        get { return this.Props.Where(kv => IsV5SupportPropType(kv.Key)); }
    }
    ...
    public int GetMaxStar()
    {
        ...
        if (this.type == GearType.machineEngine || this.type == GearType.machineArms ||
            this.type == GearType.machineLegs || this.type == GearType.machineBody ||
            this.type == GearType.machineTransistors || this.type == GearType.dragonMask ||
            this.type == GearType.dragonPendant || this.type == GearType.dragonWings ||
            this.type == GearType.dragonTail)
        {
            return 0;
        }
    ...
    public object Clone()
    {
        WzComparerR2Gear wzComparerR2Gear = (WzComparerR2Gear)this.MemberwiseClone();
        wzComparerR2Gear.Props = new Dictionary<GearPropType, int>(this.Props.Count);
        foreach (KeyValuePair<GearPropType, int> p in this.Props)
        {
            wzComparerR2Gear.Props.Add(p.Key, p.Value);
        }

        wzComparerR2Gear.Options = (WzComparerR2Potential[])this.Options.Clone();
        // wzComparerR2Gear.Additions = new List<Addition>(this.Additions);
        return wzComparerR2Gear;
    }
    ...
    public static int Compare(WzComparerR2Gear wzComparerR2Gear, WzComparerR2Gear originWzComparerR2Gear)
    {
        if (wzComparerR2Gear.ItemID != originWzComparerR2Gear.ItemID)
            return 0;
        int diff = 0;
        int tempValue;
        foreach (KeyValuePair<GearPropType, int> prop in wzComparerR2Gear.Props)
        {
            originWzComparerR2Gear.Props.TryGetValue(prop.Key, out tempValue); //在原装备中寻找属性 若没有找到 视为0
            diff += (int)Math.Round((prop.Value - tempValue) / (double)GetPropTypeWeight(prop.Key));
        }

        foreach (KeyValuePair<GearPropType, int> prop in originWzComparerR2Gear.Props)
        {
            if (!wzComparerR2Gear.Props.TryGetValue(prop.Key, out tempValue)) //寻找装备原属性里新装备没有的
            {
    ...
    public static int Compare(WzComparerR2Gear wzComparerR2Gear)
    {
        int diff = 0;
        int tempValue;
        foreach (KeyValuePair<GearPropType, int> prop in wzComparerR2Gear.Props)
        {
            wzComparerR2Gear.StandardProps.TryGetValue(prop.Key, out tempValue); //在原装备中寻找属性 若没有找到 视为0
            diff += (int)Math.Round((prop.Value - tempValue) / (double)GetPropTypeWeight(prop.Key));
        }

        foreach (KeyValuePair<GearPropType, int> prop in wzComparerR2Gear.StandardProps)
        {
            if (!wzComparerR2Gear.Props.TryGetValue(prop.Key, out tempValue)) //寻找装备原属性里新装备没有的
            {
                diff -= (int)Math.Round(prop.Value / (double)GetPropTypeWeight(prop.Key));
    ...
    public static IEnumerable<KeyValuePair<GearPropType, int>> CombineProperties(
        IEnumerable<KeyValuePair<GearPropType, int>> props)
    {
    ...
    public static IEnumerable<KeyValuePair<GearPropType, object>> CombineProperties(
        IEnumerable<KeyValuePair<GearPropType, object>> props)
    {
        ...
        foreach (var prop in propCache)
        {
            switch (prop.Key)
            {
                ...
                case GearPropType.incMMP:
                    ...
                    else if (propCache.TryGetValue(
                                 prop.Key == GearPropType.incMHP ? GearPropType.incMMP : GearPropType.incMHP, out obj)
                             && Equals(prop.Value, obj)
                             && !combinedProps.ContainsKey(GearPropType.incAllStat_incMHP50_incMMP50))
                    {
                ...
                case GearPropType.incMMPr:
                    if (combinedProps.ContainsKey(GearPropType.incMHPr_incMMPr))
                    {
                        break;
                                 ...
                                 prop.Key == GearPropType.incMHPr ? GearPropType.incMMPr : GearPropType.incMHPr,
                                 out obj)
                             && Equals(prop.Value, obj))
                    {
                ...
                case GearPropType.incMAD:
                    if (combinedProps.ContainsKey(GearPropType.incPAD_incMAD))
                    {
                        break;
                                 ...
                                 prop.Key == GearPropType.incPAD ? GearPropType.incMAD : GearPropType.incPAD, out obj)
                             && Equals(prop.Value, obj))
                    {
                ...
                case GearPropType.incMDD:
                    if (combinedProps.ContainsKey(GearPropType.incPDD_incMDD))
                    {
                        break;
                                 ...
                                 prop.Key == GearPropType.incPDD ? GearPropType.incMDD : GearPropType.incPDD, out obj)
                             && Equals(prop.Value, obj))
                    {
                ...
                case GearPropType.incEVA:
                    if (combinedProps.ContainsKey(GearPropType.incACC_incEVA))
                    {
                        break;
                                 ...
                                 prop.Key == GearPropType.incACC ? GearPropType.incEVA : GearPropType.incACC, out obj)
                             && Equals(prop.Value, obj))
                    {
    ...
    public static WzComparerR2Gear CreateFromNode(Wz_Node node, GlobalFindNodeFunction findNode)
    {
        int gearID;
        ...
        WzComparerR2Gear gear = new WzComparerR2Gear();
        gear.ItemID = gearID;
        gear.type = GetGearType(gear.ItemID);
            ...
            foreach (Wz_Node subNode in infoNode.Nodes)
            {
                switch (subNode.Text)
                {
                    case "icon":
                        ...
                        {
                            // wzComparerR2Gear.Icon = BitmapOrigin.CreateFromNode(subNode, findNode);
                        }
                    ...
                    case "iconRaw":
                        if (subNode.Value is Wz_Uol || subNode.Value is Wz_Png)
                        {
                            // wzComparerR2Gear.IconRaw = BitmapOrigin.CreateFromNode(subNode, findNode);
                        }
                    ...
                    case "sample":
                        if (subNode.Value is Wz_Uol || subNode.Value is Wz_Png)
                        {
                            // wzComparerR2Gear.Sample = BitmapOrigin.CreateFromNode(subNode, findNode);
                        }
                    ...
                    case "addition": //附加属性信息
                        ...
                        // {
                            ...
                            // if (addi != null)
                                // wzComparerR2Gear.Additions.Add(addi);
                        // }

                        // wzComparerR2Gear.Additions.Sort((add1, add2) => (int)add1.Type - (int)add2.Type);
                        break;
                    ...
                    case "option": //附加潜能信息
                        ...
                        foreach (Wz_Node optNode in subNode.Nodes)
                        {
                            ...
                            WzComparerR2Potential opt =
                                WzComparerR2Potential.CreateFromNode(itemWz.FindNodeByPath(optId.ToString("d6")),
                                    optLevel);
                            if (opt != null)
                    ...
                    case "level": //可升级信息
                        ...
                        // {
                            // wzComparerR2Gear.FixLevel = true;
                        // }

                        // Wz_Node levelInfo = subNode.Nodes["info"];
                        // wzComparerR2Gear.Levels = new List<GearLevelInfo>();
                        // if (levelInfo != null)
                        // {
                            // for (int i = 1;; i++)
                            // {
                                ...
                                // {
                                    ...
                                    // info.Level = lv;
                                    // wzComparerR2Gear.Levels.Add(info);
                                // }
                                // else
                                // {
                                    // break;
                        ...
                        // {
                            ...
                            // {
                                ...
                                // probTotal += prob;
                                // for (int i = 0; i < wzComparerR2Gear.Levels.Count; i++)
                                // {
                                    // GearLevelInfo info = wzComparerR2Gear.Levels[i];
                            ...
                            // foreach (var info in wzComparerR2Gear.Levels)
                            // {
                                // info.ProbTotal = probTotal;
                            // }
                        // }

                        // wzComparerR2Gear.Props.Add(GearPropType.level, 1);
                        break;

                    case "sealed": //封印解除信息
                        // Wz_Node sealedInfo = subNode.Nodes["info"];
                        // wzComparerR2Gear.Seals = new List<GearSealedInfo>();
                        // if (sealedInfo != null)
                        // {
                            ...
                            // {
                                ...
                                // Int32.TryParse(levelInfoNode.Text, out lv);
                                // info.Level = lv;
                                // wzComparerR2Gear.Seals.Add(info);
                            // }
                        // }

                        // wzComparerR2Gear.Props.Add(GearPropType.@sealed, 1);
                        break;
        ...
        else if (SpecialCanPotential(gear.type) || IsSubWeapon(gear.type) ||
                 (gear.Props.TryGetValue(GearPropType.tucIgnoreForPotential, out value) && value > 0))
        {
            gear.CanPotential = true;
        }
        ...
        if (gear.Props.TryGetValue(GearPropType.fixedGrade, out value))
        {
            ...
            switch (value)
            {
                case 2: gear.Grade = WzComparerR2GearGrade.B; break;
                case 3: gear.Grade = WzComparerR2GearGrade.A; break;
                case 5: gear.Grade = WzComparerR2GearGrade.S; break;
                case 7: gear.Grade = WzComparerR2GearGrade.SS; break;
                default: gear.Grade = (WzComparerR2GearGrade)(value - 1); break;
            }
        }

        //自动填充Grade
        if (gear.Options.Any(opt => opt != null) && gear.Grade == WzComparerR2GearGrade.C)
        {
            gear.Grade = WzComparerR2GearGrade.B;
        }
        ...
        //检查道具默认的剪刀次数
        var cuttableCountOverride = findNode?.Invoke(@$"Etc\KarmaScissor_WZ2.img\ItemList\{gear.ItemID}")
            ?.GetValueEx<int>();
        ...
        // {
            // gear.Icon =
                // BitmapOrigin.CreateFromNode(findNode(@"Item\Install\0380.img\03801284\info\icon"), findNode);
            // gear.IconRaw =
                // BitmapOrigin.CreateFromNode(findNode(@"Item\Install\0380.img\03801284\info\iconRaw"), findNode);
        // }

        // if (IsHair(gear.type))
        // {
            // gear.Icon =
                // BitmapOrigin.CreateFromNode(findNode(@"Item\Install\0380.img\03801283\info\icon"), findNode);
            // gear.IconRaw =
                // BitmapOrigin.CreateFromNode(findNode(@"Item\Install\0380.img\03801283\info\iconRaw"), findNode);
        // }

        // if (gear.type == GearType.head)
        // {
            // gear.Icon =
                // BitmapOrigin.CreateFromNode(findNode(@"Item\Install\0380.img\03801577\info\icon"), findNode);
            // gear.IconRaw =
                // BitmapOrigin.CreateFromNode(findNode(@"Item\Install\0380.img\03801577\info\iconRaw"), findNode);
        // }
...
public enum WzComparerR2GearGrade
{
    /// <summary>
    /// C级(一般物品)
    /// </summary>
